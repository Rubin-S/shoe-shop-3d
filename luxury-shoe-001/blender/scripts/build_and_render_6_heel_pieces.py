"""
Master Heel Lifts Multi-Shot Suite (5 Layers, 6 Individual Pieces)
Faithfully recreates authentic bespoke shoemaking construction:
- media_1789393237116.jpg (Assembled stacked heel beauty portrait)
- media_1789393748011.jpg (6-piece blueprint with locked dimensional ratios)

Generates 11 high-resolution Blender Cycles macro studio shots:
1.  Heel_Lift_Piece_01_Top_Leather.jpg
2.  Heel_Lift_Piece_02_Mid_A_Leather.jpg
3.  Heel_Lift_Piece_03_Mid_B_Leather.jpg
4.  Heel_Lift_Piece_04_Mid_C_Leather.jpg
5.  Heel_Lift_Piece_05_Bottom_Leather_Dovetail.jpg
6.  Heel_Lift_Piece_06_Rubber_Strike_Dovetail.jpg
7.  Heel_Lifts_6_Pieces_Blueprint_Layout.jpg
8.  Heel_Lifts_Exploded_Stack.jpg
9.  Heel_Lifts_Assembled_Master.jpg
10. Heel_Lifts_Dovetail_Joint_Macro.jpg
11. Piece_02_Heel_Lifts.jpg
"""
...
# ---------------------------------------------------------------------------
# 1. LOCKED PROPORTIONS (FULL HEEL LENGTH = 1.000 = 80.0 mm)
# ---------------------------------------------------------------------------
L = 0.080           # 80.0 mm reference heel length
W = 0.850 * L        # 68.0 mm heel width (ratio 0.850)
H_lift = 0.044 * L  # 3.52 mm thickness for each leather lift 1-4 (ratio 0.044)
H_bottom = 0.088 * L# 7.04 mm thickness for bottom wear layer (ratio 0.088)
gap = 0.00008       # 0.08 mm real-world physical layer gap for contact shadows

y_front = 0.475 * L
y_rear = -0.525 * L
y_seam = y_front - 0.450 * L   # 0.450 from front
y_apex = y_front - 0.575 * L   # 0.575 from front (0.125 penetration)
w_tooth = 0.260 * W            # tooth width
...
# ---------------------------------------------------------------------------
# 2. CONTOURS (ORGANIC JITTER & COMPLEMENTARY DOVETAIL CUT)
# ---------------------------------------------------------------------------
...
def get_bottom_layer_contours(scale=1.0):
    ...
    # Piece 6: Rear Rubber Strike (exact complementary cut)
    p6 = [
        (s_wsh, s_ys),
        (s_wt, s_ys),
        (0.0, s_ya),
...
def build_heel_lift_mesh(pts_bot, pts_top, z_bot, z_top, name="Lift"):
    ...
    # 0.3mm bevel with harden normals ON
...
# ---------------------------------------------------------------------------
# 4. HIGH-REALISM PBR SHADERS WITH METRIC SCALES
# ---------------------------------------------------------------------------
def create_vegtan_face_material(name, col_base):
    ...
    # Macro tanning variation (warm cognac to chestnut)
    ...
    # Leather pore bump (0.45mm pores)
    ...
    noise_rough.inputs["Scale"].default_value = 1600.0
    noise_rough.inputs["Detail"].default_value = 5.0
    links.new(tc.outputs["Object"], noise_rough.inputs["Vector"])
    ...
    map_rough.inputs["To Min"].default_value = 0.38
    map_rough.inputs["To Max"].default_value = 0.50
    links.new(noise_rough.outputs["Fac"], map_rough.inputs["Value"])
...
def create_vegtan_cut_edge_material(name, col_edge):
    ...
    # Compressed directional fibers (stretched horizontally)
...
def create_vulcanized_rubber_material():
    ...
    # High-density pebble stipple (0.25mm mold texture)
    ...
    # Roughness variation
    noise_rough = nodes.new("ShaderNodeTexNoise")
    noise_rough.inputs["Scale"].default_value = 2500.0
    noise_rough.inputs["Detail"].default_value = 5.0
    links.new(tc.outputs["Object"], noise_rough.inputs["Vector"])
    ...
    map_rough.inputs["From Min"].default_value = 0.0
    map_rough.inputs["From Max"].default_value = 1.0
    map_rough.inputs["To Min"].default_value = 0.62
    map_rough.inputs["To Max"].default_value = 0.74
    links.new(noise_rough.outputs["Fac"], map_rough.inputs["Value"])
    links.new(map_rough.outputs["Result"], bsdf.inputs["Roughness"])
...
def create_cyclorama_material():
    mat = bpy.data.materials.new(name="PBR_Cyclorama_Paper")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    links = mat.node_tree.links
    nodes.clear()
    ...
    noise.inputs["Scale"].default_value = 1800.0
...
# ---------------------------------------------------------------------------
# 5. STUDIO CYCLORAMA & 3-POINT LIGHTING
# ---------------------------------------------------------------------------
def setup_studio():
    ...
    profile = []
    for y in [-0.8, -0.4, 0.0, 0.2, 0.35]:
        profile.append((y, -0.015))
    cy = 0.35
    cz = 0.35 - 0.015
    r = 0.35
    for i in range(1, 10):
    ...
    # Key Light (Large soft area light from front-left)
    ...
    # Fill Light (Soft area light from front-right)
    ...
    # Rim Kick Light (Back-Top edge kick)
...
def setup_camera(name, loc, target_loc, lens=85.0, fstop=None):
    scene = bpy.context.scene
...
# ---------------------------------------------------------------------------
# 6. BUILD MASTER 6 PIECES
# ---------------------------------------------------------------------------
def build_all_6_pieces():
...
# ---------------------------------------------------------------------------
# 7. MULTI-SHOT RENDERING PIPELINE
# ---------------------------------------------------------------------------
def render_full_suite():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.abspath(os.path.join(script_dir, "..", ".."))
    out_dir = os.path.join(project_root, "luxury-shoe-001", "renders", "pieces", "heel_lifts")
    web_dir = os.path.join(project_root, "web", "public", "pieces", "heel_lifts")
    os.makedirs(out_dir, exist_ok=True)
    os.makedirs(web_dir, exist_ok=True)

    def save_and_copy(shot_name):
        scene = bpy.context.scene
        fpath = os.path.join(out_dir, shot_name)
        scene.render.filepath = fpath
        bpy.ops.render.render(write_still=True)
        web_path = os.path.join(web_dir, shot_name)
        shutil.copyfile(fpath, web_path)
        print(f"Delivered: {shot_name}")
    ...
    setup_studio()
    pieces = build_all_6_pieces()

    # -----------------------------------------------------------------------
    # SHOTS 1-4: INDIVIDUAL LEATHER LIFTS 1 TO 4 (STUDIO ISOLATION)
    # -----------------------------------------------------------------------
    shot_names_lifts = {
        1: "Heel_Lift_Piece_01_Top_Leather.jpg",
        2: "Heel_Lift_Piece_02_Mid_A_Leather.jpg",
    ...
    setup_camera("Single_Lift_Cam", (0.05, -0.18, 0.09), (0.0, 0.0, 0.002), lens=85.0)

    for k in range(1, 5):
        hide_all(pieces)
        p = pieces[k]
        p.hide_render = False
        # Center piece on ground
        orig_z = p.location.z
        p.location = (0.0, 0.0, -orig_z)
        p.rotation_euler = (math.radians(15), math.radians(-10), math.radians(25))
        save_and_copy(shot_names_lifts[k])

    # -----------------------------------------------------------------------
    # SHOT 5: PIECE 5 - BOTTOM LEATHER DOVETAIL (FEMALE NOTCH SHOWCASED)
    # -----------------------------------------------------------------------
    hide_all(pieces)
    p5 = pieces[5]
    p5.hide_render = False
    p5.location = (0.0, 0.0, 0.0)
    # Angled so the dovetail tongue/apex and mating face are prominently showcased
    p5.rotation_euler = (math.radians(20), math.radians(-12), math.radians(75))
    setup_camera("Piece5_Cam", (0.04, -0.18, 0.095), (0.0, 0.0, 0.004), lens=85.0)
    save_and_copy("Heel_Lift_Piece_05_Bottom_Leather_Dovetail.jpg")

    # -----------------------------------------------------------------------
    # SHOT 6: PIECE 6 - RUBBER STRIKE DOVETAIL (MALE WEDGE & PEBBLE TEXTURE)
    # -----------------------------------------------------------------------
    hide_all(pieces)
    p6 = pieces[6]
    p6.hide_render = False
    p6.location = (0.0, 0.0, 0.0)
    # Angled from front-top to showcase mating dovetail socket and matte pebble mold stippling
    p6.rotation_euler = (math.radians(22), math.radians(15), math.radians(-65))
    setup_camera("Piece6_Cam", (-0.03, -0.17, 0.09), (0.0, 0.0, 0.004), lens=85.0)
    save_and_copy("Heel_Lift_Piece_06_Rubber_Strike_Dovetail.jpg")

    # -----------------------------------------------------------------------
    # SHOT 7: 6-PIECE BLUEPRINT KNOLLING LAYOUT (MATCHING media_1789393748011.jpg)
    # -----------------------------------------------------------------------
    hide_all(pieces)
    for p in pieces.values():
        p.hide_render = False
    ...
    # Arrange in 2 rows of 3 pieces flat on the floor:
    # Row 1 (Top): Piece 1, Piece 2, Piece 3
    # Row 2 (Bottom): Piece 4, Piece 5, Piece 6
    x_spacing = 0.085
    y_spacing = 0.095
    
    pieces[1].location = (-x_spacing, y_spacing * 0.5, -0.01792)
    pieces[2].location = (0.0, y_spacing * 0.5, -0.01432)
    pieces[3].location = (x_spacing, y_spacing * 0.5, -0.01072)
    
    pieces[4].location = (-x_spacing, -y_spacing * 0.5, -0.00712)
    pieces[5].location = (0.0, -y_spacing * 0.5, 0.0)
    pieces[6].location = (x_spacing, -y_spacing * 0.5, 0.0)

    setup_camera("Knolling_Cam", (0.0, -0.16, 0.38), (0.0, 0.0, 0.0), lens=60.0)
    save_and_copy("Heel_Lifts_6_Pieces_Blueprint_Layout.jpg")

    # -----------------------------------------------------------------------
    # SHOT 8: EXPLODED STACK (VERTICAL SEPARATION & DOVETAIL UNCOUPLING)
    # -----------------------------------------------------------------------
    hide_all(pieces)
    for p in pieces.values():
        p.hide_render = False
    ...
    pieces[1].location = (0.0, 0.0, 0.075 - 0.01792)
    pieces[2].location = (0.0, 0.0, 0.055 - 0.01432)
    pieces[3].location = (0.0, 0.0, 0.035 - 0.01072)
    pieces[4].location = (0.0, 0.0, 0.018 - 0.00712)
    # Uncouple dovetail horizontally
    pieces[5].location = (0.0, 0.015, 0.0)
    pieces[6].location = (0.0, -0.015, 0.0)

    parent_expl = bpy.data.objects.new("Exploded_Parent", None)
    bpy.context.scene.collection.objects.link(parent_expl)
    for p in pieces.values():
        p.parent = parent_expl
    parent_expl.rotation_euler = (math.radians(22), math.radians(-15), math.radians(45))
    parent_expl.location = (0.0, 0.0, 0.02)

    setup_camera("Exploded_Cam", (0.05, -0.28, 0.12), (0.0, 0.0, 0.04), lens=85.0)
    save_and_copy("Heel_Lifts_Exploded_Stack.jpg")
    for p in pieces.values(): p.parent = None
    bpy.data.objects.remove(parent_expl, do_unlink=True)

    # -----------------------------------------------------------------------
    # SHOT 9: ASSEMBLED MASTER BEAUTY PORTRAIT (MATCHING media_1789393237116.jpg)
    # -----------------------------------------------------------------------
    hide_all(pieces)
    for p in pieces.values():
        p.hide_render = False
    ...
    parent_asm = bpy.data.objects.new("Assembled_Parent", None)
    bpy.context.scene.collection.objects.link(parent_asm)
    for p in pieces.values():
        p.parent = parent_asm

    parent_asm.rotation_euler = (math.radians(-28), math.radians(12), math.radians(65))
    parent_asm.location = (0.005, 0.0, 0.035)

    setup_camera("Beauty_Cam", (0.015, -0.27, 0.012), (0.005, 0.0, 0.035), lens=85.0)
    save_and_copy("Heel_Lifts_Assembled_Master.jpg")

    # -----------------------------------------------------------------------
    # SHOT 10: DOVETAIL JOINT MACRO CLOSE-UP (EXTREME CONTACT DETAIL)
    # -----------------------------------------------------------------------
    # Focus macro lens directly on the interlocking seam between leather & rubber
    # Depth of field at f/4.0
    seam_focus_loc = (0.005, -0.005, 0.028)
    setup_camera("Macro_Dovetail_Cam", (0.012, -0.16, 0.025), seam_focus_loc, lens=110.0, fstop=4.0)
    save_and_copy("Heel_Lifts_Dovetail_Joint_Macro.jpg")

    # -----------------------------------------------------------------------
    # SHOT 11: PIECE 02 CATALOG SHOWCASE (MATCHING Piece_01_Heel_Nails.jpg)
    # -----------------------------------------------------------------------
    parent_asm.rotation_euler = (math.radians(25), math.radians(-10), math.radians(45))
    parent_asm.location = (0.0, 0.0, 0.015)
    setup_camera("Catalog_Cam", (0.03, -0.25, 0.08), (0.0, 0.0, 0.015), lens=85.0)
    save_and_copy("Piece_02_Heel_Lifts.jpg")

    # Also copy to piece root directory
    catalog_root = os.path.join(project_root, "luxury-shoe-001", "renders", "pieces", "Piece_02_Heel_Lifts.jpg")
    shutil.copyfile(os.path.join(out_dir, "Piece_02_Heel_Lifts.jpg"), catalog_root)
    web_catalog_root = os.path.join(project_root, "web", "public", "pieces", "Piece_02_Heel_Lifts.jpg")
    shutil.copyfile(os.path.join(out_dir, "Piece_02_Heel_Lifts.jpg"), web_catalog_root)

    print("ALL 11 PIECE 02 SHOTS SUCCESSFULLY GENERATED AND DELIVERED!")
